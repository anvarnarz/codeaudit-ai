import Link from "next/link";

export default function LandingPage() {
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-4">
      <div className="max-w-lg w-full space-y-8 text-center">
        {/* Logo / Product name */}
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 mb-4">
            <div className="w-8 h-8 rounded-md bg-white/10 flex items-center justify-center border border-white/20">
              <span className="text-sm font-bold text-white">CA</span>
            </div>
            <span className="text-sm font-medium text-muted-foreground tracking-widest uppercase">
              CodeAudit
            </span>
          </div>

          <h1 className="text-4xl font-semibold tracking-tight text-foreground leading-tight">
            Codebase health audits,
            <br />
            <span className="text-muted-foreground">without the setup.</span>
          </h1>

          <p className="text-muted-foreground text-lg leading-relaxed">
            Connect GitHub, pick a repo, bring your own API key.
            <br />
            Get a thorough structured audit in minutes — not hours of CLI config.
          </p>
        </div>

        {/* CTA */}
        <div className="flex flex-col gap-3 items-center">
          <Link
            href="/api/auth/signin/github"
            className="inline-flex items-center gap-3 bg-white text-black font-medium px-6 py-3 rounded-lg hover:bg-white/90 transition-colors w-full max-w-xs justify-center"
          >
            <GitHubIcon />
            Sign in with GitHub
          </Link>

          <p className="text-xs text-muted-foreground">
            Bring your own Anthropic, OpenAI, or Gemini API key.
          </p>
        </div>

        {/* Features */}
        <div className="grid grid-cols-3 gap-4 pt-8 border-t border-border">
          <div className="space-y-1">
            <p className="text-sm font-medium text-foreground">13 phases</p>
            <p className="text-xs text-muted-foreground">
              Full structural audit
            </p>
          </div>
          <div className="space-y-1">
            <p className="text-sm font-medium text-foreground">3 providers</p>
            <p className="text-xs text-muted-foreground">
              Anthropic, OpenAI, Gemini
            </p>
          </div>
          <div className="space-y-1">
            <p className="text-sm font-medium text-foreground">BYOK</p>
            <p className="text-xs text-muted-foreground">You control costs</p>
          </div>
        </div>
      </div>
    </main>
  );
}

function GitHubIcon() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="18"
      height="18"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
    >
      <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
    </svg>
  );
}
